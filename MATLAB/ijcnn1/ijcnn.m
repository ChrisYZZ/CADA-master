%% ijcnn1 %%
%% SLAG - logistic regression real data
clc
clear all
close all
...
pindex = randperm(num_sample);
Xdata_28=Xdata_28(pindex,:);
ydata_28=ydata_28(pindex);
per_split = 9000*(1:10);
ratio_batch=0.01;
...
stepsize=0.1;
triggerslot=10;
thrd=1/(stepsize^2*num_workers^2)/triggerslot;%0.5
num_bits=4;
eplison = 0.00000001;
H = 10;
adam_stepsize = 0.01;
...
H = 20; % Has been declared in appendix
comm_error11=[];
comm_grad11=[];
...
beta2=0.999% beta2 = 0.9 only for FEDADAM
...
figure
semilogx(loss20,'y'); %ADAM
hold on
semilogx(loss44,'m'); %CADA1
hold on
semilogx(loss66,'k--');%CADA2
hold on
semilogx(loss13,'g'); %FED ADAM
hold on
semilogx(loss11,'b--'); %local momentum
hold on
semilogx(loss3,'m--'); %LAG


xlabel('Number of iteration','fontsize',16,'fontname','Times New Roman')
ylabel('Loss','fontsize',16,'fontname','Times New Roman')
legend('ADAM','CADA1','CADA2', 'FED ADAM','local momentum','LAG');
axis([1, 1e4, 0.12, 0.7]);
title('ijcnn');
saveas(gcf,'ijcnn_iter.pdf')


figure;

semilogx(comm_error20(:,1),abs(comm_error20(:,2)),'y'); %ADAM
hold on
semilogx(comm_error44(:,1),abs(comm_error44(:,2)),'m');%CADA1
hold on
semilogx(comm_error66(:,1),abs(comm_error66(:,2)),'k--');%CADA2
hold on;
semilogx(comm_error13(:,1),abs(comm_error13(:,2)),'g'); %FED ADAM
hold on
semilogx(comm_error11(:,1),abs(comm_error11(:,2)),'c--'); %local momentum
hold on
semilogx(comm_error3(:,1),abs(comm_error3(:,2)),'m--'); %LAG


xlabel('Number of communications (uploads)','fontsize',16,'fontname','Times New Roman')
ylabel('Loss','fontsize',16,'fontname','Times New Roman')
legend('ADAM','CADA1','CADA2', 'FED ADAM','local momentum','LAG');
axis([10, 1e5, 0.12, 0.7]);
title('ijcnn');
saveas(gcf,'ijcnn_comm.pdf')
...
semilogx(index, loss44,'m'); %CADA1
hold on
index = 2:2:length(loss66)*2;
semilogx(index, loss66,'k--');%CADA2
hold on
semilogx(loss13,'g'); %FED ADAM
hold on
semilogx(loss11,'b--'); %local momentum
hold on
semilogx(loss3,'m--'); %local lag
...
axis([1, 1e4, 0.12, 0.7]);
title('ijcnn');
saveas(gcf,'ijcnn_gradients.pdf')
