        ...
        try:
            with torch.cuda.device(device):
                ...
                if delay[i] >= delay_bound:
                    ...
                    if method == 'wk1':
                        delta_new = check_lag_wk1(grad, grad_old, delta[i], thrd)
...
""" LSGD """
def parallel_apply_LSGD(models, zipdata_list, criterion, weights, lr, devices=None):
    lock = threading.Lock()
    grad_enabled = torch.is_grad_enabled()
...
def parallel_apply_LMSGD(models, mts_w, zipdata_list, criterion, weights, lr, momentum, weight_decay, devices=None):
    lock = threading.Lock()
    grad_enabled = torch.is_grad_enabled()
    H = len(zipdata_list[0])
    num_workers = len(zipdata_list)

    def _worker(i, model, mt, data_list, weight, device=None):
        ...
        with torch.cuda.device(device):
            for data in data_list:
                ...
                loss.backward()
                grad = get_grad(model, weight)
                update_params_lmsgd(model, grad, mt, lr*num_workers, momentum, weight_decay)


    threads = [threading.Thread(target=_worker, 
                                args=(i, model, mt, data_list, weight, device)) 
               for i, (model, mt, data_list, weight, device) in 
               enumerate(zip(models, mts_w, zipdata_list, weights, devices))]
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
